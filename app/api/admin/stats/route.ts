...
export async function GET() {
  ...
  const [{ count: totalUsers }, { count: activeUsers }, { data: users }, { count: activeMarkers }] =
    await Promise.all([
      ...
      db.from('dps_markers').select('*', { count: 'exact', head: true }).gt('expires_at', new Date().toISOString())
    ])

  const byPlatform: Record<string, number> = { telegram: 0, vk: 0, max: 0, web: 0 }
