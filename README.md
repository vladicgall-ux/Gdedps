...
Telegram Mini App и (в перспективе) внутри MAX. Стек: **Next.js 14
...
- Клик по метке открывает карточку: автор, время, комментарии, кнопка «Всё ещё там» (продлевает жизнь метки на 3 часа).
- Метка автоматически исчезает через 3 часа, если её никто не подтвердил.
- Авторизация: Telegram (6-значный код через бота + Mini App initData), MAX (заготовка под phone-share), веб — кнопки на `/login`.
...
app/
  ...
  api/
    auth/telegram|max/route.ts   проверка подписи провайдера, upsert пользователя, выдача сессии
    ...
    markers/[id]/confirm/route.ts   POST «всё ещё там» (+3 часа)
...
components/
  ...
  LoginButtons.tsx    кнопки Telegram / MAX для веб-версии
...
lib/
  ...
  auth/telegram.ts, max.ts        верификация подписи каждой платформы
  auth/session.ts, upsertUser.ts  JWT-сессия в httpOnly cookie, upsert профиля
  platform.ts                     определение среды (Telegram/MAX/веб)
...
| `TELEGRAM_BOT_TOKEN`, `NEXT_PUBLIC_TELEGRAM_BOT_USERNAME` | бот, созданный через [@BotFather](https://t.me/BotFather) (нужен и для кода входа, и для Mini App) |
...
2. Внутри Mini App `window.Telegram.WebApp.initData` уходит на `/api/auth/telegram` и проверяется HMAC-подписью с `TELEGRAM_BOT_TOKEN` — это уже реализовано (`app/login/page.tsx`, `lib/auth/telegram.ts`). Для веб-версии используется 6-значный код входа через бота (`app/api/auth/telegram/code/*`).
...
## 8. Автоудаление меток через 3 часа

- Каждая метка создаётся с `expires_at = now() + 3 часа`.
- Кнопка «Всё ещё там» в карточке метки (`/api/markers/[id]/confirm`) продлевает `expires_at` ещё на 3 часа.
...
- `/admin` показывает: всего пользователей, активных за 24 часа, разбивку по платформам (Telegram/MAX/Веб), список активных меток с быстрым удалением.
...
| CSP | `next.config.mjs` задаёт `frame-ancestors` только для конкретных доменов Telegram/MAX + `'self'`, без wildcard; плюс `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`. |
...
| Секреты | Токены Telegram/MAX, `SUPABASE_SERVICE_ROLE_KEY` и `SESSION_SECRET` читаются только на сервере (`lib/supabase/server.ts`, `lib/auth/*`) и никогда не попадают в клиентский бандл — в `NEXT_PUBLIC_*` вынесены только несекретные идентификаторы (app id, имя бота). |
